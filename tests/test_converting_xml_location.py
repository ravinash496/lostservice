...
class LocationTest(unittest.TestCase):
    def test_location_civic_address(self):
        ...
        self.assertEqual(result.id, 'ce152f4b-2ade-4e37-9741-b6649e2d87a6')
        self.assertEqual(result.profile, 'civic')
        self.assertEqual(result.location.country, 'US')
        self.assertEqual(result.location.a1, 'ME')
        self.assertEqual(result.location.a2, 'Piscataquis')
        self.assertEqual(result.location.a3, 'T3 R11 Wels')
        self.assertEqual(result.location.rd, 'Golden')
        self.assertEqual(result.location.sts, 'Rd')
        self.assertEqual(result.location.hno, '5833')
        self.assertEqual(result.location.pc, '00135')
    ...
    def test_location_circle(self):
        ...
        target = LocationXmlConverter()
        result = target.parse(root)
        self.assertEqual(result.id, '24b276e1-7eca-48d5-be24-30e0dd4c1ca0')
        self.assertEqual(result.profile, 'geodetic-2d')
        self.assertEqual(result.location.spatial_ref, 'urn:ogc:def:crs:EPSG::4326')
